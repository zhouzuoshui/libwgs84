...
#define rad_to_deg 57.29577951308238
...
{
    double cmeridian;
    cmeridian = (-183.0 + (( double )zone * 6.0)) * deg_to_rad;
...
double FootpointLatitude(double y)
{
    ...
    result =
        y_ + (beta_ * sin(2.0 * y_)) + (gamma_ * sin(4.0 * y_)) + (delta_ * sin(6.0 * y_)) + (epsilon_ * sin(8.0 * y_));
...
void MapXYToLatLon(double x, double y, double lambda0, double *phi, double *lambda)
{
    ...
    /* Calculate latitude */
    *phi = phif + x2frac * x2poly * (x * x) + x4frac * x4poly * pow(x, 4.0) + x6frac * x6poly * pow(x, 6.0)
           + x8frac * x8poly * pow(x, 8.0);

    /* Calculate longitude */
    *lambda = lambda0 + x1frac * x + x3frac * x3poly * pow(x, 3.0) + x5frac * x5poly * pow(x, 5.0)
